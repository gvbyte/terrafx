...
parser = argparse.ArgumentParser(
    description='Export a SignalFx asset as Terraform')
parser.add_argument('--key', dest='key', required=True,
                    help='An API key for accessing SignalFx')
parser.add_argument('--api_url', dest='api_url',
                    help='The API URL, used for non-default realms', default='https://api.us1.signalfx.com')
parser.add_argument('--name', dest='name', required=True,
                    help='The name of the resource after export, e.g. mychart0')
parser.add_argument('--exclude', dest='excludes', nargs='*',
                    help='A field to exclude from the emitted HCL', default=['id', 'url', 'tags'])
parser.add_argument('--output', dest='output', required=True,
                    help='The name of the directory to which output will be written')
group = parser.add_mutually_exclusive_group(required=True)
group.add_argument('--group', dest='group',
                   help='The ID of the dashboard group in SignalFx')
group.add_argument('--dashboard', dest='dash',
                   help='The ID of the dashboard in SignalFx')
group.add_argument('--detector', dest='detector',
                   help='The ID of the detector in SignalFx')
group.add_argument('--globaldatalink', dest='globaldatalink',
                   help='The colon : seperated property name and value for a global data link - if value is wildcarded just provide the name')
...
with signalfx.SignalFx(
    ...
    if args['group']:
        ...
        output = handle_asset(args['key'], args['api_url'],
                              "signalfx_dashboard_group", args['name'], args['group'])
        if output != None:
            write_output(args['output'], args['name'] + ".tf",
        ...
        write_output(args['output'], args['name'] +
                     ".tf", det_out.decode('utf-8'))
    elif args['globaldatalink']:
        det_out = handle_datalink(
            sfx, args['globaldatalink'], args['name'], args)
        write_output(args['output'], args['name'] +
                     ".tf", det_out.decode('utf-8'))
    else:
